...
function shortWeather(results: any) {
  const shortWeather: any = [];
  Object.values(results).forEach((result: any) => {
    ...
    const temp_max_min: any = {
      temperature_max: [],
      temperature_min: [],
    };
    const precipitation_max_min: any = {
      precipitation_max: [],
      precipitation_min: [],
    };
    ...
    result.forEach((dayResult) => {
      ...
      if (
        Number(hourObject) <= Number(HourNow) &&
        Number(dayObject) === Number(dayNow)
      ) {
        /* empty */
      } else {
        const tempValue: any = dayResult?.data.instant?.details.air_temperature;
        ...
        if (Number(time_day) === Number(time_now)) {
          ...
          day.time =
            time_day + " " + dayNames[new Date(dayResult.time).getUTCDay()];
        }
        switch (hourObject) {
          case "00":
            day.icon.night = dayResult?.data.next_6_hours?.summary.symbol_code;
            day.precipitation.night =
              dayResult?.data.next_6_hours?.details.precipitation_amount;
            break;
          case "06":
            day.icon.morning =
              dayResult?.data.next_6_hours?.summary.symbol_code;
            day.precipitation.morning =
              dayResult?.data.next_6_hours?.details.precipitation_amount;
            break;
          case "12":
            day.icon.midday = dayResult?.data.next_6_hours?.summary.symbol_code;
            day.precipitation.midday =
              dayResult?.data.next_6_hours?.details.precipitation_amount;
            break;
          case "18":
            day.icon.evening =
              dayResult?.data.next_6_hours?.summary.symbol_code;
            day.precipitation.evening =
              dayResult?.data.next_6_hours?.details.precipitation_amount;
            break;
          default:
            break;
...
function longWeather(results: any) {
  const forecast: any = [];
  ...
  Object.values(results).forEach((result: any) => {
    ...
    result.forEach((dayResult) => {
      ...
      if (
        Number(time.substring(11, 13)) <= getHours &&
        Number(time.substring(8, 10)) === getDate
      ) {
        /* empty */
      } else {
        hour.time = time;
        ...
        hour.cloud.cloud_area_fraction_medium =
          details.cloud_area_fraction_medium;
