    ...
    <div class="weather-container" :class="timeseries ? '' : 'blur'">
      <div v-for="(item, index) in short" :key="item.shortWeather">
        ...
        >
          <summary
            v-if="
              item?.temperature_max && item?.temperature_min !== 'undefined'
            "
            ...
            <div class="summary">
              <div v-for="(icon, time) in item.icon" :key="time">
                <div v-if="icon !== undefined" :class="time">
                  ...
                  />
                  <div class="precipitation">
                    <b v-if="item.precipitation">{{
                      item.precipitation[time]
                    }}</b>
                    <p v-if="item.precipitation">
                      {{ unitsObject.precipitation }}
                    </p>
                  </div>
                </div>
          ...
          >
            <div v-if="forecast_data.icon !== 'none'">
              ...
              <div class="time">
                <b class="hour">{{
                  forecast_data.time.split("T", 7)[1].slice(0, -1)
                }}</b>
              </div>
              <h3 class="temp">
                <b>{{
                  forecast_data ? forecast_data.temperature : "Loading..."
                }}</b>
                <p>{{ unitsObject.temperature }}</p>
              </h3>
              <div class="pressure">
                <b>{{
                  forecast_data ? forecast_data.pressure : "Loading..."
                }}</b>
                <p>{{ unitsObject.pressure }}</p>
              </div>
              <div class="precipitation">
                <b>{{
                  forecast_data ? forecast_data.precipitation : "Loading..."
                }}</b>
              ...
              <div class="wind">
                ...
                <IconArrow
                  :style="{
                    rotate:
                      forecast_data && forecast_data.wind
                        ? forecast_data.wind.direction +
                          unitsObject.wind.direction
                        : '0deg',
                  }"
                />
              </div>
